...
RUN apt-get update && apt-get install -y --no-install-recommends \
  ...
  && HOST_ARG="" \
  && if [ "${TARGETARCH}" = "amd64" ]; then HOST_ARG="--host=x86_64-linux-gnu"; fi \
  && if [ "${TARGETARCH}" = "arm64" ]; then HOST_ARG="--host=aarch64-linux-gnu"; fi \
  && if [ "${TARGETARCH}" = "armv7" ]; then HOST_ARG="--host=arm-linux-gnueabihf"; fi \
  && if [ "${TARGETARCH}" = "i386" ]; then HOST_ARG="--host=i686-linux-gnu"; fi \
  && ./configure --prefix=/usr --sysconfdir=/etc --disable-local-only --enable-shared --with-tls=openssl --disable-systemd ${HOST_ARG} \
  ...
  && apt-get purge -y build-essential git pkg-config libgnutls28-dev libavahi-client-dev libdbus-1-dev zlib1g-dev libssl-dev \
...
RUN mkdir -p /var/log/cups /var/cache/cups /var/spool/cups /var/run/cups /etc/cups/ssl /usr/etc/cups \
