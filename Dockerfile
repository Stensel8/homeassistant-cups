...
# Combine all RUN commands into one layer to reduce image size
# Install dependencies, bashio, and build CUPS in single layer
RUN apt-get update && apt-get install -y --no-install-recommends \
  ...
  # Configure with architecture detection
  && case "${TARGETARCH}" in \
  ...
  # Clean up build artifacts and apt cache
...
# Create directories, set permissions, create symlinks in one layer
